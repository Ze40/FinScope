...
export async function loader() {
  return await getData({ pageParam: 1, tableName: "goverment" });
}
...
const Goverment = () => {
  ...
  const handleLoadMore = async () => {
    ...
    if (nextPage && !isLoading) {
      ...
      try {
        const newPage = await getData({ pageParam: nextPage, tableName: "goverment" });
  ...
  return (
    <>
      ...
      {tableData.rows.length > 0 && (
        <DataTable
          tableIdField="goverment_id"
          data={tableData}
          tableLabel="Государства"
          tableName="goverment"
