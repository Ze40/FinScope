...
export async function loader() {
  return await getData({ pageParam: 1, tableName: "production" });
}
...
const Production = () => {
  ...
  const handleLoadMore = async () => {
    ...
    if (nextPage && !isLoading) {
      ...
      try {
        const newPage = await getData({ pageParam: nextPage, tableName: "production" });
  ...
  return (
    <>
      ...
      {tableData.rows.length > 0 && (
        <DataTable
          tableIdField="production_id"
          data={tableData}
          tableLabel="Производства"
